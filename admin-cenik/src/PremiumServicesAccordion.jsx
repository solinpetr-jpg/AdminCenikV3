...
import { premiumServices } from './data/premiumServices.js'
...
function PremiumServiceRow({ service, quantity, setQuantity }) {
  const { addOrUpdateItem, updateItemQuantity, getItemQuantity } = useCart()
  const hasQuantityOptions = service.quantityOptions?.length > 0
  const [selectedQuantityIndex, setSelectedQuantityIndex] = useState(hasQuantityOptions ? null : null)
  const showStepper = quantity > 0

  // Synchronizace s košíkem
  ...
  const minQtyForDiscount = hasQuantityOptions && service.quantityOptions?.length
    ? Math.min(...service.quantityOptions.map((opt) => quantityFromLabel(opt.label)))
    : 0
  const showAddMoreForDiscount = hasQuantityOptions && quantity > 0 && quantity < minQtyForDiscount
  // Sleva se počítá jen pokud má služba quantityOptions
  ...
  const handleQuantityClick = (i) => {
    if (!hasQuantityOptions) return
  ...
  const updateCartQuantity = (newQuantity) => {
    if (newQuantity === 0) {
      ...
      if (cartQuantity === 0) {
        ...
        updateItemQuantity(`premium-${service.id}`, newQuantity)
      }
    }
  }
  ...
  const handleAddClick = () => {
    let newQuantity
    if (hasQuantityOptions && selectedQuantityIndex != null) {
      // Pokud je vybrána varianta, použij ji
      newQuantity = quantityFromLabel(service.quantityOptions[selectedQuantityIndex].label)
      setQuantity(newQuantity)
    } else {
      // Pokud není vybrána varianta, přidej 1 kus (výchozí hodnota)
      newQuantity = 1
      setQuantity(newQuantity)
      // Nevybíráme žádnou variantu (zůstane null)
    }
    updateCartQuantity(newQuantity)
  }

  const syncSelectionFromStepper = (value) => {
    ...
    >
      ...
      <div className="premium-service-row-title">
        <div className="d-flex align-items-center gap-2">
          ...
          {cartQuantity > 0 ? (
            <span className="premium-service-order-badge">
              Přidáno
            </span>
          ) : (
      ...
      {/* Pravý blok: grid sloupce sdílené všemi řádky – discount-label vždy stejné X */}
      <div className="premium-service-row-right">
        ...
        {hasQuantityOptions ? (
          <div className="d-flex flex-wrap gap-2 align-items-center flex-shrink-0" role="group" aria-label="Množství">
        ...
        <div className="premium-service-price-block text-end">
          {/* Řádek původní celkové ceny - vždy přítomen, aby výška neskákala */}
          <div className="package-card-price-old" style={{ visibility: discountedPrice != null ? 'visible' : 'hidden' }}>
            {formatPriceCZK(totalPrice)}
          </div>
          <div className="d-flex align-items-center justify-content-end gap-2">
            <span className="package-card-price">{formatPriceCZK(discountedPrice ?? (quantity > 0 ? totalPrice : service.priceCZK))}</span>
            <PriceInfoPopover
              title={service.title}
        ...
        {/* Wrapper s fixní šířkou pro CTA/stepper, aby spacing neskákal */}
...
export default function PremiumServicesAccordion() {
  ...
  return (
    <div className={`premium-accordion rounded overflow-hidden shadow-sm ${expanded ? 'premium-accordion--open' : ''}`}>
      <button
        type="button"
      ...
      >
        <div className="d-flex align-items-center gap-3">
          ...
          <h2 className="premium-section-title mb-0">Prémiové služby pro Práci</h2>
        </div>
        <div className="d-flex align-items-center gap-2">
          <span className="premium-section-summary">
            {premiumServices.length} {pluralPoložka(premiumServices.length)} • {selectedCount} vybráno
          </span>
          <svg
            width="20"
            height="20"
