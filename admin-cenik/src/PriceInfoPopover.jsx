...
/** Portál vždy do body – panel je přímý potomek body, takže ho neořezává žádný accordion. */
...
/**
 * Pozice panelu vůči triggeru (jako v TOP kartách), ale s clampingem do viewportu.
 * Obsah jde přes Portal do kontejneru na body, takže není ořezáván accordionem.
 */
function usePanelPosition(triggerRef, isOpen) {
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const panelW = Math.min(440, vw - 24) // stejná šířka jako v CSS panelu
      const margin = 16

      // Panel vždy vlevo od ikonky, vodorovně zarovnaný tak,
      // aby byl "vycentrovaný" k tlačítku (ikonce).
      let left = rect.left - panelW - margin
      left = Math.max(margin, Math.min(vw - panelW - margin, left))

      // Svisle panel vycentrujeme podle středu ikonky
      // a přicvakneme do viewportu, aby byl celý vidět.
...
export default function PriceInfoPopover({ title, unitPrice, hasQuantityOptions = true, maxQuantity = 20 }) {
  ...
  const open = () => {
    ...
    if (typeof document !== 'undefined') {
      document.body.dataset.priceInfoOpen = (Number(document.body.dataset.priceInfoOpen || 0) + 1).toString()
